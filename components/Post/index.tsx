...
const Post = ({ compact = false, post, className = "" }: PostProps) => {
	...
	const property = (
		name: string,
		El: ComponentType<PropertyComponentProps>,
	) => {
		if (post.properties[name] && Array.isArray(post.properties[name])) {
			return post.properties[name].map((value: any, i: number) => (
			...
			<Card>
				{(postFeatured || postName) && (
					<header
						...
						style={
							postFeatured
								? { backgroundImage: `url(${postFeatured})` }
								: undefined
						}
					>
						{!!postFeatured && (
